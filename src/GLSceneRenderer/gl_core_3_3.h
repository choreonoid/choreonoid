...
/*
 * This file was originally generated by glLoadGen (OpenGL Loader Generator).
 *
 * MODIFICATIONS:
 * - Added ogl_LoadFunctionsEGL() for EGL support (Linux only)
 *   Loads OpenGL function pointers using eglGetProcAddress instead of glXGetProcAddressARB
 * - Added duplicate load prevention (tracks if functions already loaded)
 * - Added conflict detection (prevents mixing GLX and EGL in same process)
 *
 * NOTES:
 * - GLSceneRenderer.so is a shared library, so function pointers are shared
 *   across all users (Base, GLVisionSimulatorPlugin, etc.)
 * - Currently, either GLX or EGL can be used per process, not both simultaneously
 *
 * FUTURE IMPROVEMENT:
 * - Consider migrating to GLAD (https://github.com/Dav1dde/glad)
 *   GLAD is actively maintained and natively supports EGL/GLX/WGL
 * - If concurrent GLX/EGL use is needed, convert GLSceneRenderer to static library
 *   so each module can have independent function pointers
 */
