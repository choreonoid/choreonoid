...
PortConnectionDialogBaseExt::PortConnectionDialogBaseExt()
    : isAccepted(false)
{
...
void PortConnectionDialogBaseExt::onAddButtonClicked()
{
...
void PortConnectionDialogBaseExt::onDeleteButtonClicked()
{
...
void PortConnectionDialogBaseExt::enableDetails(bool on)
{
...
void PortConnectionDialogBaseExt::addProperty(string name, string value)
{
    NamedValueExtPtr param(new NamedValueExt(name, value));
    propList.push_back(param);
}


DataPortConnectionDialogExt::DataPortConnectionDialogExt()
{
...
void DataPortConnectionDialogExt::setDisp(RTSPortExt* source, RTSPortExt* target)
{
    DDEBUG("CreateConnectionDialog::setDisp");
    nameLineEdit->setText(QString((source->name + "_" + target->name).c_str()));
    //
    vector<string> types = RTCCommonUtilExt::getAllowDataTypes(source, target);
    ...
    //
    types = RTCCommonUtilExt::getAllowInterfaceTypes(source, target);
    ...
    //
    types = RTCCommonUtilExt::getAllowDataflowTypes(source, target);
    dataflowCombo->clear();
    for (int index = 0; index < types.size(); index++) {
    ...
    //
    types = RTCCommonUtilExt::getAllowSubscriptionTypes(source, target);
...
void DataPortConnectionDialogExt::subscriptionComboSelectionChanged(int target)
{
...
void DataPortConnectionDialogExt::pushpolicyComboSelectionChanged(int target)
{
...
void DataPortConnectionDialogExt::onOkButtonClicked()
{
...
ServicePortConnectionDialogExt::ServicePortConnectionDialogExt()
{
...
void ServicePortConnectionDialogExt::setDisp(RTSPortExt* source, RTSPortExt* target)
{
    nameLineEdit->setText(QString((source->name + "_" + target->name).c_str()));
...
void ServicePortConnectionDialogExt::registInterfaceMap(RTSPortExt* port)
{
    ...
    for (int index = 0; index < port->interList.size(); index++) {
        PortInterfaceExtPtr ip = port->interList[index];
...
void ServicePortConnectionDialogExt::addIF()
{
...
void ServicePortConnectionDialogExt::deleteIF()
{
...
void ServicePortConnectionDialogExt::onOkButtonClicked()
{
