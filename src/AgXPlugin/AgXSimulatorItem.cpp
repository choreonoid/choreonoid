        ...
        if(extractor->extract(link->collisionShape(), std::bind(&AgXLink::addMesh, this, extractor, agxBody))){
            if(!vertices.empty()){
                ...
                if(link->jointType() == Link::PSEUDO_CONTINUOUS_TRACK || link->jointType() == Link::CRAWLER_JOINT){
                    ...
                    crawlerGeometry->setSurfaceVelocity( agx::Vec3f(1,0,0) );   //驕ｩ蠖薙↓險ｭ螳壹＠縺ｦ縺翫°縺ｪ縺・→calculateSurfaceVelocity縺悟他縺ｳ蜃ｺ縺輔ｌ縺ｪ縺・・                    agxRigidBody->add( crawlerGeometry );
                }else{
...
{
    // 縺薙％縺ｧagxSimulation縺ｮ繝｡繧ｽ繝・ラ繧貞他縺ｶ縺ｨ邨ゆｺ・凾縺ｫ繧ｨ繝ｩ繝ｼ縺檎匱逕溘☆繧九Ｂgx::shutdown()縺悟ｮ溯｡後＆繧後◆蠕後・縺溘ａ縲・
}
