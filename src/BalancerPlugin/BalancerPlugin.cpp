...
QWidget* BalancerPanel::panel()
{
    return this;
}


QHBoxLayout* BalancerPanel::newRow(QVBoxLayout* vbox)
{
...
bool BalancerPanel::apply(BodyItem* bodyItem, PoseProvider* provider, BodyMotionItem* motionItem, bool putMessages)
{
    ...
    balancer->setTimeMargins(
        timeToStartBalancerSpin.value(), bar->preInitialDuration(), bar->postFinalDuration());
    ...
    balancer->setBoundarySmoother(
        boundarySmootherCombo.currentIndex(), boundarySmootherTimeSpin.value());
    balancer->enableBoundaryCmAdjustment(
        boundaryCmAdjustmentCheck.isChecked(), boundaryCmAdjustmentTimeSpin.value());
...
void BalancerPanel::storeState(Archive* archive)
{
...
void BalancerPanel::restoreState(const Archive* archive)
{
